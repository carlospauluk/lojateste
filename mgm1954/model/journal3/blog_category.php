...
class ModelJournal3BlogCategory extends Model {
	...
	public function all($filters = array()) {
		...
		$sql = "
			FROM
				`{$this->dbPrefix('journal3_blog_category')}` {$filter_sql} c
			LEFT JOIN 
				`{$this->dbPrefix('journal3_blog_category_description')}` {$filter_sql} cd ON c.category_id = cd.category_id
			WHERE
		...
		if ($count) {
			$query = $this->db->query("SELECT c.category_id, cd.name {$sql} GROUP BY c.`category_id`");
