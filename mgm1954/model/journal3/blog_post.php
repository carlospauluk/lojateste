...
class ModelJournal3BlogPost extends Model {
	...
	public function all($filters = array()) {
		...
		$sql = "
			FROM
				`{$this->dbPrefix('journal3_blog_post')}` {$filter_sql} p
			LEFT JOIN 
				`{$this->dbPrefix('journal3_blog_post_description')}` {$filter_sql} pd ON p.post_id = pd.post_id
			WHERE
		...
		if ($count) {
			$query = $this->db->query("SELECT p.post_id, pd.name {$sql} GROUP BY p.`post_id`");
